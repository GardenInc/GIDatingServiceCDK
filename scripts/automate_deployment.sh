...
# - Set TOOLS_ACCOUNT_ID, BETA_ACCOUNT_ID, and PROD_ACCOUNT_ID env variables
...
if [[ -z "${TOOLS_ACCOUNT_ID}" || -z "${BETA_ACCOUNT_ID}" || -z "${PROD_ACCOUNT_ID}" ]]; then
...
aws cloudformation deploy --template-file cfnRolesTemplates/CodePipelineCrossAccountRole.yml \
    ...
    --profile beta \
    --parameter-overrides ToolsAccountID=${TOOLS_ACCOUNT_ID} Stage=Beta &
...
aws cloudformation deploy --template-file cfnRolesTemplates/CodePipelineCrossAccountRole.yml \
    ...
    --capabilities CAPABILITY_NAMED_IAM \
    --profile beta \
    --parameter-overrides ToolsAccountID=${TOOLS_ACCOUNT_ID} Stage=Beta KeyArn=${KEY_ARN} &
...
printf "\n  aws cloudformation describe-stacks --stack-name BetaApplicationDeploymentStack \
